...
class ProductsSystem {
    ...
    // Load products from localStorage (admin managed) or fallback to defaults
    loadProducts() {
        if (window.PaintsData) {
            this.products = window.PaintsData.getProductsFromStorage();
            if (!this.products.length) {
                this.products = window.PaintsData.initProductStorage();
            }
        } else {
    ...
    loadDefaultProducts() {
        this.products = window.PaintsData ? window.PaintsData.DEFAULT_PRODUCTS.map(window.PaintsData.normalizeProduct) : [
            {
                id: 1,
    ...
    // Refresh products from localStorage (call this when returning from admin panel)
    refreshProducts() {
        this.loadProducts();
        this.applyFilters();
    }
    ...
    setupStorageListener() {
        window.addEventListener('storage', (e) => {
            if (e.key === 'adminProducts' || e.key === 'products') {
                console.log('Products updated from admin panel, refreshing...');
    ...
    showProductDetail(productId) {
        const sameId = window.PaintsData
            ? window.PaintsData.sameId
            : (a, b) => String(a) === String(b);
        const product = this.products.find(p => sameId(p.id, productId));
        if (!product) return;
                ...
                <div class="product-detail-grid">
                    <div class="product-images">
                        <div class="main-image">
                            <img src="${product.image}" alt="${product.title}" id="mainProductImage" onerror="this.src='images/placeholder.jpg'">
                        </div>
                        <div class="image-thumbnails">
                            ${galleryImages.map((img, index) => `
                                <img src="${img}" alt="${product.title} ${index + 1}" 
                                     class="thumbnail ${index === 0 ? 'active' : ''}"
                                     onclick="products.changeMainImage(${JSON.stringify(img)}, this)"
