        ...
        orderCard.innerHTML = `
            <div class="order-header">
                ...
                <div class="order-total">
                    <span class="total-amount">${this.formatOrderPrice(order.pricing.total)}</span>
                </div>
            </div>
                ...
                ${order.items.map(item => `
                    <div class="order-item">
                        <div class="item-info">
                            ...
                            <p>Quantity: ${item.quantity} × ${this.formatOrderPrice(item.price)}</p>
                        </div>
                        <div class="item-price">
                            ${this.formatOrderPrice(item.price * item.quantity)}
                        </div>
                    </div>
                `).join('')}
            </div>
    ...
    applyFilters() {
        ...
        if (this.searchQuery) {
            filtered = filtered.filter(order => 
                ...
                order.items.some(item => (item.name || item.title || '').toLowerCase().includes(this.searchQuery))
            );
        }
                        ...
                        <div class="items-list">
                            ${order.items.map(item => `
                                <div class="detail-item">
                                    <div class="item-info">
                                        <h5>${item.name || item.title || 'Item'}</h5>
                    ...
                    <div class="details-section">
                        ...
                        <div class="summary-list">
                            <div class="summary-row">
                                <span>Subtotal:</span>
                                <span>${this.formatOrderPrice(order.pricing.subtotal)}</span>
                            </div>
                            <div class="summary-row">
                                <span>Tax:</span>
                                <span>${this.formatOrderPrice(order.pricing.tax)}</span>
                            </div>
                            <div class="summary-row">
                                <span>Shipping:</span>
                                <span>${order.pricing.shipping === 0 ? 'FREE' : this.formatOrderPrice(order.pricing.shipping)}</span>
                            </div>
                            ${order.pricing.discount > 0 ? `
                                <div class="summary-row discount">
                                    <span>Discount:</span>
                                    <span>-${this.formatOrderPrice(order.pricing.discount)}</span>
                                </div>
                            ` : ''}
                            <div class="summary-row total">
                                <span>Total:</span>
                                <span>${this.formatOrderPrice(order.pricing.total)}</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        return `
            ...
            <body>
                ...
                <div class="section">
                    ...
                    ${order.items.map(item => `
                        <div class="item">
                            <span>${item.name || item.title || 'Item'} (Qty: ${item.quantity})</span>
                            <span>${this.formatOrderPrice(item.price * item.quantity)}</span>
                        </div>
                    `).join('')}
                </div>
