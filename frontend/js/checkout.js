...
class CheckoutManager {
    ...
    saveOrder(order) {
        ...
        if (auth.isLoggedIn()) {
            ...
            // Sync to users array for admin panel
            ...
            if (userIndex !== -1) {
                const adminOrder = {
                    ...
                    productName: order.items.map(i => i.name || i.title).filter(Boolean).join(', '),
                ...
                users[userIndex].orders.push(adminOrder);
                localStorage.setItem('users', JSON.stringify(users));

                const updatedUser = { ...users[userIndex] };
                delete updatedUser.password;
                const stored = localStorage.getItem('currentUser') ? localStorage : sessionStorage;
                stored.setItem('currentUser', JSON.stringify(updatedUser));
    ...
    // Simulate UPI payment (demo functionality)
    simulateUPIPayment() {
        ...
        setTimeout(() => {
            showToast('UPI payment successful! Proceeding to next step...', 'success');
            this.nextStep();
        }, 2000);
    }
}
