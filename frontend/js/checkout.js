// UPI Scanner Functions
function showUPIScanner() {
    const scannerSection = document.getElementById('upiScannerSection');
    if (scannerSection) {
...
function updateUPIAmount() {
    ...
    if (amountElement && window.checkoutManager) {
        const total = window.checkoutManager.calculateTotal();
        amountElement.textContent = `₹${total.toLocaleString()}`;
    }
}

function copyUPIId() {
    const upiId = 'paintsworks@upi';
...
function simulateUPISuccess() {
    ...
    // Hide scanner after successful payment
    setTimeout(() => {
        hideUPIScanner();
        
        // Proceed to next step if checkout manager exists
    ...
    async processOnlinePayment() {
        try {
            ...
            const orderData = {
                ...
                address: `${this.formData.address}, ${this.formData.city}, ${this.formData.state} - ${this.formData.zipCode}`,
            ...
            const result = await razorpayPayment.initializePayment(
            ...
            // Payment will be handled by Razorpay modal
            // The rest of the flow continues in handlePaymentSuccess
